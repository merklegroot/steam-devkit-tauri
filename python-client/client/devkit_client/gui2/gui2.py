...
class DevkitCommands:
    ...
    def _update_game(self, devkit, restart_steam, gdbserver, steam_play, steam_play_debug, steam_play_debug_version, *args):
        ...
        if new_game_args.name.lower() == 'steam' and restart_steam:
            if not devkit.is_steamOS:
                # pretty rare, worth a mention
                logger.info('Remote is not running SteamOS - ignoring restart steam flag')
            else:
                # sideloaded Steam client:
...
class Devkit:
    ...
    # Implies that all devkit functionality is available
    # Early versions of the devkit tool supported partial functionality via devkit service installs on generic linux
    # (the old mode is deprecated)
    @property
    def is_steamOS(self):
        return self.steamos_status.get('is_steamos', False)
    ...
    def tick(self, visible):
        ...
        if len(online_kits) == 0:
            ...
            for kit in online_kits:
                ...
                if kit.is_steamOS:
                    description += f' SteamOS: {kit.os_version}'
                else:
                    description += f' OS: {kit.os_name}'
                if kit.is_steamOS and not kit.user_password_is_set:
            ...
            if self.selected_devkit is not None:
                if self.selected_devkit.is_steamOS:
                    active_buttons = [
                    ...
                    # Non SteamOS functionality is deprecated
                    active_buttons = [
                        RefreshStatus.BUTTON_NAME,
                        ListTitles.BUTTON_NAME,
                        RemoteShell.BUTTON_NAME,
                ...
                if self.selected_devkit.is_steamOS:
                    ...
                    if steamos_status['has_side_loaded_client']:
                        config_options.append( ( 'sideloaded client', 'SteamStatus.SIDE' ) )
                    status_lookup = steamos_status['steam_status']
                    if status_lookup == 'SteamStatus.VSCODE':
                        status_lookup = 'SteamStatus.SIDE' # don't distinguish, just say 'sideloaded client'
                ...
                # deprecated: subtools that would work against generic linux
                subtool_list += [
...
class UpdateTitle(ToolWindow):
    ...
    def tick(self, visible):
        ...
        if self.devkits_window.selected_devkit.is_steamOS and self.title_name.lower() == 'steam':
            imgui.text('Restart:')
            imgui.next_column()
            clicked, v = imgui.checkbox('Restart Steam sideloaded client on upload', self.restart_steam)
            imgui.next_column()
            if clicked:
    ...
    def do_upload(self):
        if not os.path.isdir(self.local_folder):
            ...
            self.modal_wait = ModalWait(
                self.viewport,
                self.toolbar,
                'Source folder for upload is misconfigured',
                failed_future,
            )
            return
    ...
    def on_build_success(self, name):
        ...
        if name == 'steam':
            ...
            if client_config != 'SteamConfig.SIDE':
                ...
                failed_future = concurrent.futures.Future()
                failed_future.set_exception(Exception(f'Received a build success notification for {name!r}, but the device is not configured to run a sideloaded Steam client ({client_config}), stopping.'))
                self.modal_wait = ModalWait(
                    self.viewport,
                    self.toolbar,
